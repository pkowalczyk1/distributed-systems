...
# Iterate and call invoke() method by random callers and keep track of who
# called it.
...
for _ in range(5):
    ...
    print(f"Named caller {random_name_invoker} produced {value}")

for name in CALLERS:
    values_produced = ray.get(worker_invoker.get_invoker_values.remote(name))
    print(f"Named caller {name} produced values: {values_produced}")
